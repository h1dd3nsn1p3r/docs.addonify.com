---
title: Wishlist Page Options
sidebarTitle: Wishlist Page
---

# Wishlist Page


## Wishlist Page Options

### Text when the wishlist is empty

This feature allows you to customize the text of the wishlist page when the wishlist is empty.

### Display a link

Enable this feature to display a link to the wishlist page when wishlist is empty.


### Page to be linked

Select the page to be linked when wishlist is empty.


### Link label

This feature allows you to change the label of the link to the wishlist page when wishlist is empty.


## Button to Empty Wishlist Options

### Display button to empty wishlist

By enabling this feature user can empty the wishlist by clicking an one button. This button will be displayed on the wishlist page when wishlist is not empty.

[![clear wishlist](/products/wishlist/clear-wishlist.png)](/products/wishlist/clear-wishlist.png)


### Button label

This feature allows you to change the label of the button to empty wishlist.
